    ...
    public function processOrder($order_data, $create = true) {
        ...
        if (!empty($order_data['shipping_code'])) {
            ...
            if (isset( $shippingCode[1] )) {
                $parts = explode('-', $shippingCode[1]);

                if ( isset( $parts[1] ) ) {
                    $tarif_id = (int)trim( end($parts) );
                }
            }
        ...
        */
        ...
        if ( $tarif_id ) {
            $order['delivery']['data'] = array(
                'tariff' => $tarif_id,
                'payerType' => 'sender',
                'packages' => array()
            );
        
            if ( !empty($this->session->data['shiptor']['address']['point_id']) ) {
                $order['delivery']['data']['pickuppointId'] = $this->session->data['shiptor']['address']['point_id'];
            }
        }
