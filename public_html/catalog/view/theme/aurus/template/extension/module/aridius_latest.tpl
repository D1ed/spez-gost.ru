...
<?php if ($carusel != '1') { ?>
<div id="latest-grid" class="bl_margin flex_height_row">
   <?php foreach ($products as $product) { ?>
   <div class="product-layout <?php if ($column_left || $column_right ) { ?> nth1 <?php } else { ?> nth2 <?php } ?> latest <?php if ($column_left || $column_right ) { ?> col-xl-five col-lg-3 col-md-4 col-sm-4 col-xs-6 <?php } else { ?> col-xl-2 col-lg-five col-md-3 col-sm-4 col-xs-6 <?php } ?>">
      <div class="gtile-i-wrap">
         <div class="gtile-i">
            <div class="gtile-i-box">
               <div class="item_lat caption">
...
<script>
   ...
   var size_li_lat = $("#latest-grid > div").size();
   var li_element_lat  = <?php if ($column_left || $column_right ) { ?>5<?php } else { ?>6<?php } ?>;
   if(li_element_lat !=size_li_lat){
   $( ".product-layout.latest:nth-child(<?php if ($column_left || $column_right ) { ?>5<?php } else { ?>6<?php } ?>n):not(:last-child)" ).append('<div class="gridcount latest"><i class="gridcount_load fa fa-refresh fa-lg"></i><span class="gridcount2"><?php echo $button_load; ?></span></div>');
   $(".product-layout.latest:nth-child(<?php if ($column_left || $column_right ) { ?>5<?php } else { ?>6<?php } ?>n):not(:last-child)").addClass("latest_box quickview_hidden");
   }
   $('#latest-grid > div:lt('+li_element_lat+')').fadeIn('slow');
   $('.gridcount.latest').click(function () {
   var bLazy = new Blazy()
   $(this).remove();
   $('.latest_box.quickview_hidden').first().removeClass('quickview_hidden')
   if(li_element_lat==size_li_lat){
   $('.btn.btn-default.gridcount.latest').remove();
   }else{
   li_element_lat = (li_element_lat+<?php if ($column_left || $column_right ) { ?>5<?php } else { ?>6<?php } ?> <= size_li_lat) ? li_element_lat+<?php if ($column_left || $column_right ) { ?>5<?php } else { ?>6<?php } ?> : size_li_lat;
   $('#latest-grid > div:lt('+li_element_lat+')').fadeIn('slow');
   }
...
<?php } else { ?>
<div id="latest_carusel<?php echo $module; ?>" class="swiper-container">
   ...
   <?php if ($navigation) { ?>
   <div class="#latest_carusel<?php echo $module; ?> swiper-button-next"><i class="fa fa-chevron-right fa-5x"></i></div>
   <div class="#latest_carusel<?php echo $module; ?> swiper-button-prev"><i class="fa fa-chevron-left fa-5x"></i></div>
   <?php } ?>
</div>
<script><!--
   $(document).ready(function () {
   var ocSwiper = new Swiper('#latest_carusel<?php echo $module; ?>.swiper-container', {
   ...
   <?php if ($navigation) { ?>	
       navigation: {
           nextEl: '#latest_carusel<?php echo $module; ?> .swiper-button-next',
           prevEl: '#latest_carusel<?php echo $module; ?> .swiper-button-prev',
       },
   <?php } ?>	
   });
   <?php if ($stophover && $autoplay_on) { ?>
   $("#latest_carusel<?php echo $module; ?>.swiper-container").hover(function() {
