...
<?php if ($carusel != '1') { ?>
<div id="bestseller-grid" class="bl_margin flex_height_row">
   <?php foreach ($products as $product) { ?>
   <div class="product-layout <?php if ($column_left || $column_right ) { ?> nth1 <?php } else { ?> nth2 <?php } ?> bestseller <?php if ($column_left || $column_right ) { ?> col-xl-five col-lg-3 col-md-4 col-sm-4 col-xs-6 <?php } else { ?> col-xl-2 col-lg-five col-md-3 col-sm-4 col-xs-6 <?php } ?>">
...
<script>
   ...
   var size_li_best = $("#bestseller-grid > div").size();
   var li_element_best  = <?php if ($column_left || $column_right ) { ?>5<?php } else { ?>6<?php } ?>;
   if(li_element_best !=size_li_best){
   $( ".product-layout.bestseller:nth-child(<?php if ($column_left || $column_right ) { ?>5<?php } else { ?>6<?php } ?>n):not(:last-child)" ).append('<div class="gridcount bestseller"><i class="gridcount_load fa fa-refresh fa-lg"></i><span class="gridcount2"><?php echo $button_load; ?></span></div>');
   $(".product-layout.bestseller:nth-child(<?php if ($column_left || $column_right ) { ?>5<?php } else { ?>6<?php } ?>n):not(:last-child)").addClass("bestseller_box quickview_hidden");
   }
   $('#bestseller-grid > div:lt('+li_element_best+')').fadeIn('slow');
   $('.gridcount.bestseller').click(function () {
   $(this).remove();
      $('.bestseller_box.quickview_hidden').first().removeClass('quickview_hidden')
   if(li_element_best==size_li_best){
   $('.btn.btn-default.gridcount.bestseller').remove();
   }else{
...
<?php } else { ?>
<div id="bestseller_carusel<?php echo $module; ?>" class="swiper-container">
   ...
   <?php if ($navigation) { ?>
   <div class="#bestseller_carusel<?php echo $module; ?> swiper-button-next"><i class="fa fa-chevron-right fa-5x"></i></div>
   <div class="#bestseller_carusel<?php echo $module; ?> swiper-button-prev"><i class="fa fa-chevron-left fa-5x"></i></div>
   <?php } ?>
</div>
<script><!--
   $(document).ready(function () {
   var ocSwiper = new Swiper('#bestseller_carusel<?php echo $module; ?>.swiper-container', {
   ...
   <?php if ($navigation) { ?>	
       navigation: {
           nextEl: '#bestseller_carusel<?php echo $module; ?> .swiper-button-next',
           prevEl: '#bestseller_carusel<?php echo $module; ?> .swiper-button-prev',
       },
   <?php } ?>	
   });
   <?php if ($stophover && $autoplay_on) { ?>
   $("#bestseller_carusel<?php echo $module; ?>.swiper-container").hover(function() {
