...
class ControllerExtensionModulefishSingleclick extends Controller {
    private $error = array();
                ...
                private $_landing_info = array();

                protected function getLandingField($field) {
                    return isset($this->_landing_info[$field]) ? $this->_landing_info[$field] : null;
                }
    ...
    public function __construct($registry) {
        ...
        $this->language->load('extension/module/fish_singleclick');
                ...
                public function fishcontent($setting) {
                    $this->_landing_info = $setting;
                //Multi Fish Landing 2.3.x End
                ...
                //Multi Fish Landing 2.3.x Begin
                for ($x = 0; $x < 2; $x++) {
                    if (is_file(DIR_TEMPLATE . 'default/stylesheet/fishstylesheet' . $x . '.css')) {
                        $data['file_version_fishstylesheet'][$x] = date('Y.m.d.H.i.s', filemtime(DIR_TEMPLATE . 'default/stylesheet/fishstylesheet' . $x . '.css'));
                    }
                }

                if (is_file(DIR_TEMPLATE . 'default/stylesheet/multifishstylesheet.css')) {
                    $data['file_version_multifishstylesheet'] = date('Y.m.d.H.i.s', filemtime(DIR_TEMPLATE . 'default/stylesheet/multifishstylesheet.css'));
                }
                //Multi Fish Landing 2.3.x End
            ...
            $data['is_capcha']        = $this->getLandingField('fish_singleclick_capcha');
            $data['phone_mask']       = trim($this->getLandingField('fish_singleclick_phone_mask'));
            $data['order_status_id']  = $this->getLandingField('fish_singleclick_default_order_status');
		...
		if (is_file(DIR_IMAGE . $this->config->get('config_logo'))) {
			...
			$data['og_url'] = (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1')) ? HTTPS_SERVER : HTTP_SERVER) . substr($this->request->server['REQUEST_URI'], 1, (strlen($this->request->server['REQUEST_URI'])-1));
                ...
                //Multi Fish Landing 2.3.x Begin
                $data['name'] = $this->getLandingField('meta_title');
                $data['logo'] = ($this->getLandingField('land_logo') != 'no_image.png') && is_file(DIR_IMAGE . $this->getLandingField('land_logo')) ? $server . 'image/' . $this->getLandingField('land_logo') : '';
                //Multi Fish Landing 2.3.x End
			...
			$fish_singleclick = $this->getLandingField('fish_singleclick');
//print_r('<pre>');
...
$data['showl']  = $this->getLandingField('fish_singleclick_showl');
$data['showstyle']  = $this->getLandingField('fish_singleclick_style');
$data['titlefont']  = $this->getLandingField('fish_singleclick_font1');
$data['bodyfont']  = $this->getLandingField('fish_singleclick_font2');
$data['titlefonth1_size']  = $this->getLandingField('fish_singleclick_fonth1_size');
$data['titlefonth1_weigh']  = $this->getLandingField('fish_singleclick_fonth1_weigh');
$data['titlefonth1_trans']  = $this->getLandingField('fish_singleclick_fonth1_trans');
$data['titlefonth2_size']  = $this->getLandingField('fish_singleclick_fonth2_size');
$data['titlefonth2_weigh']  = $this->getLandingField('fish_singleclick_fonth2_weigh');
$data['titlefonth2_trans']  = $this->getLandingField('fish_singleclick_fonth2_trans');
$data['titlefonth3_size']  = $this->getLandingField('fish_singleclick_fonth3_size');
$data['titlefonth3_weigh']  = $this->getLandingField('fish_singleclick_fonth3_weigh');
$data['titlefonth3_trans']  = $this->getLandingField('fish_singleclick_fonth3_trans');
$data['bodyfont_size']  = $this->getLandingField('fish_singleclick_fontbody_size');
$colormenu  = $this->getLandingField('fish_singleclick_colormenu');
			if ($colormenu !='') { $data['colormenu'] = $colormenu; } else {  $data['colormenu'] = 0;}
$colormenutext  = $this->getLandingField('fish_singleclick_colormenutext');
			if ($colormenutext !='') { $data['colormenutext'] = $colormenutext; } else {  $data['colormenutext'] = 0;}
$colorfooter  = $this->getLandingField('fish_singleclick_colorfooter');
			if ($colorfooter !='') { $data['colorfooter'] = $colorfooter; } else {  $data['colorfooter'] = 0;}
$colorfootertext  = $this->getLandingField('fish_singleclick_colorfootertext');
			if ($colorfootertext !='') { $data['colorfootertext'] = $colorfootertext; } else {  $data['colorfootertext'] = 0;}
$bodyfon  = $this->getLandingField('fish_singleclick_bodyfon');
			if ($bodyfon !='') { $data['bodyfon'] = $bodyfon; } else {  $data['bodyfon'] = 0;}
			

//menu
$data['showmenu']  = $this->getLandingField('fish_singleclick_showmenu');
$data['showmenu2']  = $this->getLandingField('fish_singleclick_showmenu2');
...
$data['topmenu8'] = isset($fish_singleclick['fish_singleclick_menu8'][$this->config->get('config_language_id')]) ? html_entity_decode($fish_singleclick['fish_singleclick_menu8'][$this->config->get('config_language_id')], ENT_QUOTES, 'UTF-8') : 0;
   		   $fish_headerlink = $this->getLandingField('fish_singleclick_headerlink');
			   if(isset($fish_headerlink)){
				foreach ($fish_headerlink as $result) {
				    $data['fish_headerlink'][] = array(
					'text' => isset($result['text'][$this->config->get('config_language_id')]) ? nl2br($result['text'][$this->config->get('config_language_id')]): '',
			...
			//slider
			   ...
			   $fish_slider = $this->getLandingField('fish_singleclick_img');
			...
			$fish_singleclicks = $this->getLandingField('fish_singleclicks');
			$data['fish_singleclick_phone'] = html_entity_decode($fish_singleclicks['fish_singleclick_phone'], ENT_QUOTES, 'UTF-8');
			
			//Sale
			$data['showsale']  = $this->getLandingField('fish_singleclick_showsale');
			$data['showsale2']  = $this->getLandingField('fish_singleclick_showsale2');
			$colorsale  = $this->getLandingField('fish_singleclick_colorsale');
			if ($colorsale !='') { $data['colorsale'] = $colorsale; } else {  $data['colorsale'] = 0;}
			 $colorsaletext  = $this->getLandingField('fish_singleclick_colorsaletext');
			...
			if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) {
				...
				$fish_singleclick_imgfon_sale = $this->getLandingField('fish_singleclick_imgfon_sale');   
			...
			$fish_singleclick_img_sale = $this->getLandingField('fish_singleclick_img_sale');   
			...
			$data['year']  = $this->getLandingField('fish_singleclick_year');
			$data['month']  = $this->getLandingField('fish_singleclick_month');
			$data['day']  = $this->getLandingField('fish_singleclick_day');
			
	  	//advantages
		$data['showadv']  = $this->getLandingField('fish_singleclick_showadv');
			$coloradv  = $this->getLandingField('fish_singleclick_coloradv');
			if ($coloradv !='') { $data['coloradv'] = $coloradv; } else {  $data['coloradv'] = 0;}
			$coloradvtext  = $this->getLandingField('fish_singleclick_coloradvtext');
			if ($coloradvtext !='') { $data['coloradvtext'] = $coloradvtext; } else {  $data['coloradvtext'] = 0;}
			
			   
				$fish_singleclick_img_adv = $this->getLandingField('fish_singleclick_img_adv');   
		...
		$data['advtext'] = isset($fish_singleclick['fish_singleclick_advtext'][$this->config->get('config_language_id')]) ? html_entity_decode($fish_singleclick['fish_singleclick_advtext'][$this->config->get('config_language_id')], ENT_QUOTES, 'UTF-8') : '';
		 if (isset($fish_singleclick['fish_adv_width']) && ($fish_singleclick['fish_adv_width'] != '')) {
			   ...
			   $fish_adv = $this->getLandingField('fish_singleclick_adv');
			   if(isset($fish_adv)){
			  ...
			  //products
		$data['showprod']  = $this->getLandingField('fish_singleclick_showprod');
			$colorprod  = $this->getLandingField('fish_singleclick_colorprod');
			if ($colorprod !='') { $data['colorprod'] = $colorprod; } else {  $data['colorprod'] = 0;}
			$colorprodtext  = $this->getLandingField('fish_singleclick_colorprodtext');
			if ($colorprodtext !='') { $data['colorprodtext'] = $colorprodtext; } else {  $data['colorprodtext'] = 0;}
			
			$columnprod  = $this->getLandingField('fish_singleclick_column');
			...
			if ($data['columnprod'] < 12) {$data['columnprod2'] = 6;} else {$data['columnprod2'] = 12;}
				...
				$fish_singleclick_img_prod = $this->getLandingField('fish_singleclick_img_prod');   
		...
		$data['prodtext'] = isset($fish_singleclick['fish_singleclick_prodtext'][$this->config->get('config_language_id')]) ? html_entity_decode($fish_singleclick['fish_singleclick_prodtext'][$this->config->get('config_language_id')], ENT_QUOTES, 'UTF-8') : '';
		 if (isset($fish_singleclick['fish_prod_width']) && ($fish_singleclick['fish_prod_width'] != '')) {
			   ...
			   $fish_prod = $this->getLandingField('fish_singleclick_prod');
			   if(isset($fish_prod)){
	  ...
	  //products BD
		$data['showproda']  = $this->getLandingField('fish_singleclick_showproda');
			$colorproda  = $this->getLandingField('fish_singleclick_colorproda');
			if ($colorproda !='') { $data['colorproda'] = $colorproda; } else {  $data['colorproda'] = 0;}
			$colorprodtexta  = $this->getLandingField('fish_singleclick_colorprodtexta');
			if ($colorprodtexta !='') { $data['colorprodtexta'] = $colorprodtexta; } else {  $data['colorprodtexta'] = 0;}
			
			$columnproda  = $this->getLandingField('fish_singleclick_columna');
			...
			if ($data['columnproda'] < 12) {$data['columnprod2a'] = 6;} else {$data['columnprod2a'] = 12;}
				...
				$fish_singleclick_img_proda = $this->getLandingField('fish_singleclick_img_proda');   
		...
		$data['products'] = array();
        $fish_proda = $this->getLandingField('fish_singleclick_product'); 
		 ...
		 if(isset($fish_proda)){
			...
			foreach ($products as $product_id) {
			 ...
			 //text blocks  
			 $colortextbox  = $this->getLandingField('fish_singleclick_colortextbox');
		 ...
		 if (isset($fish_singleclick['fish_box_width']) && ($fish_singleclick['fish_box_width'] != '')) {
			   ...
			   $fish_box = $this->getLandingField('fish_singleclick_box');
			   if(isset($fish_box)){
				foreach ($fish_box as $result) {
				    ...
				    $data['fish_box'][] = array(
					'text' => isset($result['text'][$this->config->get('config_language_id')]) ? nl2br($result['text'][$this->config->get('config_language_id')]) : 0,
			...
			//reviews
		$data['showreview']  = $this->getLandingField('fish_singleclick_showreview');
			$colorreview  = $this->getLandingField('fish_singleclick_colorreview');
			if ($colorreview !='') { $data['colorreview'] = $colorreview; } else {  $data['colorreview'] = 0;}
			  $colorreviewtext  = $this->getLandingField('fish_singleclick_colorreviewtext');
			if ($colorreviewtext !='') { $data['colorreviewtext'] = $colorreviewtext; } else {  $data['colorreviewtext'] = 0;}
			
			$columnreview  = $this->getLandingField('fish_singleclick_columnreview');
			...
			if ($data['columnreview'] < 12) {$data['columnreview2'] = 6;} else {$data['columnreview2'] = 12;}
				...
				$fish_singleclick_img_review = $this->getLandingField('fish_singleclick_img_review');   
		...
		$data['reviewtext'] = isset($fish_singleclick['fish_singleclick_reviewtext'][$this->config->get('config_language_id')]) ? html_entity_decode($fish_singleclick['fish_singleclick_reviewtext'][$this->config->get('config_language_id')], ENT_QUOTES, 'UTF-8') : '';
		 if (isset($fish_singleclick['fish_review_width']) && ($fish_singleclick['fish_review_width'] != '')) {
			   ...
			   $fish_review = $this->getLandingField('fish_singleclick_review');
...
//carousel
		$data['showcarousel']  = $this->getLandingField('fish_singleclick_showcarousel');
			$colorcarousel  = $this->getLandingField('fish_singleclick_colorcarousel');
			if ($colorcarousel !='') { $data['colorcarousel'] = $colorcarousel; } else {  $data['colorcarousel'] = 0;}
			  $colorcarouseltext  = $this->getLandingField('fish_singleclick_colorcarouseltext');
			if ($colorcarouseltext !='') { $data['colorcarouseltext'] = $colorcarouseltext; } else {  $data['colorcarouseltext'] = 0;}
			
			$columncarousel  = $this->getLandingField('fish_singleclick_columncarousel');
			...
			if ($data['columncarousel'] < 4) {$data['columncarousel2'] = $data['columncarousel']; $data['columncarousel3'] = $data['columncarousel'];} 
				...
				$fish_singleclick_img_carousel = $this->getLandingField('fish_singleclick_img_carousel');   
		...
		$data['carouseltext2'] = isset($fish_singleclick['fish_singleclick_carouseltext2'][$this->config->get('config_language_id')]) ? html_entity_decode($fish_singleclick['fish_singleclick_carouseltext2'][$this->config->get('config_language_id')], ENT_QUOTES, 'UTF-8') : '';
		 if (isset($fish_singleclick['fish_carousel_width']) && ($fish_singleclick['fish_carousel_width'] != '') && ($fish_singleclick['fish_carousel_width'] != '0')) {
             ...
             if (($fish_carousel_bigwidth) && ($fish_carousel_bigheight)) { $data['showbigimage'] = true;} else {$data['showbigimage'] = false;}
			   ...
			   $fish_carousel = $this->getLandingField('fish_singleclick_carousel');
	...
	//customs
		$data['showcustom']  = $this->getLandingField('fish_singleclick_showcustom');
			$colorcustom  = $this->getLandingField('fish_singleclick_colorcustom');
			if ($colorcustom !='') { $data['colorcustom'] = $colorcustom; } else {  $data['colorcustom'] = 0;}
          $colorcustomtext  = $this->getLandingField('fish_singleclick_colorcustomtext');
			if ($colorcustomtext !='') { $data['colorcustomtext'] = $colorcustomtext; } else {  $data['colorcustomtext'] = 0;}
			   
				$fish_singleclick_img_custom = $this->getLandingField('fish_singleclick_img_custom');   
			...
			$fish_singleclick_img_iconcustom = $this->getLandingField('fish_singleclick_img_iconcustom');   
		...
		$data['customtext3'] = isset($fish_singleclick['fish_singleclick_customtext3'][$this->config->get('config_language_id')]) ? html_entity_decode($fish_singleclick['fish_singleclick_customtext3'][$this->config->get('config_language_id')], ENT_QUOTES, 'UTF-8') : ''; 			   	      	   
		   ...
		   $fish_netw = $this->getLandingField('fish_singleclick_netw');
			   if(isset($fish_netw)){
	   ...
	   if ((strlen($data['customcontact1']) > 11) && (strlen($data['customcontact2']) > 11)) {$data['customclass'] = '6';} else {$data['customclass'] = '12';}	
	   		   $fish_footerlink = $this->getLandingField('fish_singleclick_footerlink');
    ...
    protected function validate() {
        ...
        if ($this->getLandingField('fish_singleclick_capcha')) {
