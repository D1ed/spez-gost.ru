...
class ControllerCatalogProduct extends Controller {
	...
	public function index() {
				...
				$query = $this->db->query("DESC " . DB_PREFIX . "product_description marketplace_description");
				if (!$query->num_rows) { 
					$this->db->query("ALTER TABLE `" . DB_PREFIX . "product_description` ADD `marketplace_description` text COLLATE utf8_bin NOT NULL");
				}
    ...
    if ($this->config->get('config_editor_default')) {
		...
		$data['entry_description'] = $this->language->get('entry_description');
			...
			$data['entry_marketplace_description'] = $this->language->get('entry_marketplace_description');
...
// FX CC →
				$data['canonical_name'] = '';			
    			$first = current($data['product_description']);
    			
    			if (ctype_digit((string)$first['meta_keyword'])){
    				$canonical_info = $this->model_catalog_product->getProduct((int)$first['meta_keyword']);
    				if ($canonical_info) $data['canonical_name'] = $canonical_info['name'];
    			}
// ← FX CC
			...
			if (isset($this->error['marketplace_description'])) {
				$data['marketplace_description'] = $this->error['marketplace_description'];
			} else {
				$data['marketplace_description'] = array();
			}
