...
class ControllerYamarketFusionFeed extends \yamarket_fusion\Module\controllers\module\Feed {
	...
	private function getYML() {
		...
		$categories = array_filter($this->categories, function($el) {return (bool)$el['status'];});
		...
		while ($products = $this->model_catalog_catalog->getProducts($products_query_data)) {
			...
			foreach ($products as $product) {
				if ($this->setting('market_product_available')
				...
				if ($this->setting('market_raz') == 1) {
					...
					if (isset($db_filter_field[$this->setting('market_type')])
						&& isset($product[$db_filter_field[$this->setting('market_type')]])
						&& !$product[$db_filter_field[$this->setting('market_type')]]
					) {
						continue;
					}
				}
				...
				if ($this->setting('market_marketplace_description') == 0) {
					...
					$offerNode->setDescription(html_entity_decode(html_entity_decode($product['marketplace_description'], ENT_QUOTES)));
				}
				...
				foreach ($product_images as $i => &$image) {
					...
					if ($this->setting('market_picture') == 1) {
						$image = HTTPS_SERVER . 'image/'.$image;
					}else{
						$image = str_replace(array('&amp;', ' '), array('&', '%20'), $this->model_tool_image->resize($image, $offerNode::image_width, $offerNode::image_height));
					}
	...
	protected function addUrlUtm($offer, $category_name = '') {
		...
		$url_params['utm_campaign'] = YandexUrlHelper::encodeUtmContent(StringHelper::transliterate($product['name']));
		...
		foreach ($market_delivery_options as $market_option) {
			...
			if ($weight_from_len || $weight_to_len) {
				...
				if (!MathHelper::inRange(
					$this->weight->convert($offerNode->getWeight(), $product['weight_class_id'], $market_option['weight_class']), $weight_from,
					$weight_to
				)) { continue; }
			}
			...
			if (!empty($market_option['category'])) {
				...
				else if ($offerNode->categoryId != $market_option['category']
	...
	private function addPurchasePriceMarkup($offerNode, $product) {
		...
		foreach ($markup_options as $markup_option) {
			...
			if (!empty($markup_option['category'])) {
				...
				else if ($offerNode->categoryId != $markup_option['category']
	...
	private function addOfferCustomTags($offer, $product) {
		foreach ($this->setting('custom_market_tags', array()) as $ctag) {
			...
			if (!empty($ctag['filter_category_id'])) {
				...
				else if ($offer->categoryId != $ctag['filter_category_id']
			...
			while ($market_products = $this->model_module->getMarketOffers($profile_id, $offers_query_data)) {
				...
				if ($market_products) {
					...
					foreach ($compare_offers as $offer) {
						...
						if ((float)$offer['price'] != (float)$market_product['price'] 
			...
			foreach ($offers_to_update as $profile_id => $value) {
				if ($this->setting('products_price_autoupdate_cron_type_direct', null, $profile_id) == 0) {
					...
					foreach ($all_offers as $offers) {
						...
						if (!$result->getErrors()) {
							$updated = array_uintersect($offers, $result->getData(), function($a, $b) {return $a['product_id'] == $b['product_id'] ? 0 : -1;});
