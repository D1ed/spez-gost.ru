...
class Core extends \Controller {
	...
	public function getView($add = false) {
		...
		if (version_compare(VERSION, '2.2', '<')) {
			...
			return $route;
		} elseif (version_compare(VERSION, '3.1', '<')) {
			return $route . '_3_0';
		} else {
			return $route . '_3_1';
		}
	...
	public function validate($route = '') {
		...
		if (!$this->user->hasPermission('modify', ($route == '' ? $this->getRoute() : $route)) || (isset($this->request->post[$this->getPrefix() . '_license']) && isset($this->request->server['HTTP_HOST']) && base64_encode(hash_hmac('sha256',$this->request->server['HTTP_HOST'].$this->code,M_PI,true)) != $this->request->post[$this->getPrefix() . '_license'])) {
	 ...
	 */
	public function curl($method = '', $params = [], $header = 'json', $post = true) {
		switch ($header) {
			case 'json':
		...
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, $this->config->get($this->getPrefix() . '_timeout'));
		...
		if (!is_array($result) && $this->getPrefix() == 'll_lpost_exchange') {
		...
		// cdek
		if ($info['content_type'] == 'text/xml; charset=UTF-8' || $info['content_type'] == 'application/xml;charset=UTF-8' || $info['content_type'] == 'application/xml') {
	...
	public function getCache($method, $postfix = '') {
		if ($this->config->get($this->getPrefix() . '_cache')) {
			return $this->cache->get($this->code . '.' . $method . '.' . base64_encode($postfix));
		}
	}

	public function setCache($method, $postfix = '', $data) {
		if ($this->config->get($this->getPrefix() . '_cache')) {
